...
class Edge(object):
    ...
    def __init__(self, local_master_rank, dest, src, local_rank):
        ...
        if local_master_rank in [self.src, self.dest] and local_rank == 0:
            initializer_tensor = torch.Tensor([1]).to(torch.device("cuda:{}".format(local_master_rank)))
            dist.all_reduce(initializer_tensor, group=self.process_group)
            initializer_tensor = torch.Tensor([1]).to(torch.device("cuda:{}".format(local_master_rank))).half()
            dist.all_reduce(initializer_tensor, group=self.process_group)
